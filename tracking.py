...
class IOUModelTracker:
    ...
    def _predict_next_detection(self):
        if len(self.center_queue) == 1:
            return self.center_queue[-1]

        current_model = self._choose_right_model()
        return current_model.predict(self.center_queue)
    ...
    def update_with_estimation(self):
        next_prediction = self._predict_next_detection()
        self.center_queue.append(next_prediction)

    def get_state(self):
        return {'detection': self.center_queue[-1], 'color': self.color}
...
if __name__ == "__main__":
    uber_tracker = IOUModelTracker(
        {'center': [100, 100], 'box': [[50, 50], [150, 150]]}
    )

    print(uber_tracker.get_estimation())

    for measurement in (
        {'center': [100, 98.0], 'box': [[50, 50], [150, 150]]},
        {'center': [105.0, 120.0], 'box': [[50, 50], [150, 150]]},
        {'center': [107.0, 144.3], 'box': [[50, 50], [150, 150]]},
        {'center': [108.0, 161.0], 'box': [[50, 50], [150, 150]]},
        {'center': [108.0, 190.0], 'box': [[50, 50], [150, 150]]},
        {'center': [108.0, 190.0], 'box': [[50, 50], [150, 150]]},
        {'center': [108.0, 190.0], 'box': [[50, 50], [150, 150]]}
    ):
